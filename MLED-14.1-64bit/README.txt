...
Grab the set of tagfiles from the server:
  ...
  # wget http://www.microlinux.fr/slackware/MLED-14.1-64bit/tagfiles.tar.gz 
...
convenient for handling third-party repositories like MLED. 
  ...
  # links http://www.microlinux.fr/slackware/MLED-14.1-64bit/slackware
...
now's the time to eventually catch up on it. The 'MLED-14.1-64bit/tools/'
...
The script makes use of 'slackpkg', so make sure it's configured correctly.
  ...
  # cd slackware/MLED-14.1-64bit/tools/ 
...
Besides that, your 'gcc-*' and 'glibc-*' packages will be replaced by Multilib
versions. Again, don't worry, this is also on purpose. That's because
'slackpkg+' has been preconfigured to make use of Eric Hameleers' Multilib
packages. This enables you to easily run applications like VirtualBox, Wine,
Skype, etc.

The initial system upgrade we've just performed has already replaced your base
'gcc-*' and 'glibc-*' packages by the respective Multilib versions. The only
thing that's left to do now is to install the complete set of additional
32-bit support libraries. This can be done in one simple step using the
following command:
